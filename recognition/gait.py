...
def displayMob(name, frame):
    ...
    for detection in detections:
        bbox = frameNorm(frame, (detection.xmin, detection.ymin, detection.xmax, detection.ymax))
        cv2.putText(frame, labelMap[detection.label], (bbox[0] + 10, bbox[1] + 20), font, 0.5, color)
        cv2.putText(frame, f"{int(detection.confidence * 100)}%", (bbox[0] + 10, bbox[1] + 40), font, 0.5, color)
        cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
        cv2.putText(frame, "NN fps: {:.2f}".format(nn_counter / (monotonic() - startTime)), (2, frame.shape[0] - 4), font, 0.4, (255,255,255))
    # Show the frame
    cv2.imshow(name, frame)
...
while True:
    ...
    # if inRgb is not None:
        ...
        # cam_time = 1/(monotonic() - cam_fps)
        # cam_fps_mean = (cam_fps_mean + cam_time)/2
        # cv2.putText(frame, "Fps: {:.2f}".format(cam_fps_mean),
        #                 (2, frame.shape[0] - 15), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255,255,255))
        # cam_fps = monotonic()
